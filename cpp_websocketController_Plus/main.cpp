...
=== PLUS ===
clipboard text: 05  any and all clipboard text starting from second byte
**/
...
void on_message(server* s, websocketpp::connection_hdl hdl, message_ptr msg) {
	...
	if (inl > 2){
		...
		else if (in[0] == 1)
		{
			...
			string new_clipboard = in.substr(1);
	...
	case WM_ACTIVATE:{
						 ...
						 **/
						 if (wParam == WA_INACTIVE){
							 ...
							 // HEY YOU
							 // and delete justMinimized var at top. mkay thx :)


							 //if (justMinimized){ // after minimize, window will regain focus for some reason via WA_ACTIVE... this is fix
								 //justMinimized = false;
							 //}
							 //else{
							 hasFocus = false;
...
LRESULT CALLBACK LowLevelKeyboardProc(INT nCode, WPARAM wParam, LPARAM lParam){
	if (!hasFocus) return 0; // only check the keys and take over IF window has focus. bit of an inconvenience otherwise >.>
